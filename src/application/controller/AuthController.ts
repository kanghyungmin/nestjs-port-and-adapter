...
import { HttpLoggedInUser, HttpRequestWithAppleUser, HttpRequestWithKaKaoUser, HttpRequestWithUser, HttpUserPayload } from '@application/auth/type/HttpAuthTypes'
...
import { HttpAppleAuthGuard } from '@application/auth/guard/HttpAppleAuthGuard'
...
export class AuthController {
  ...
  @Get('apple/callback')
  @HttpCode(HttpStatus.OK)
  @UseGuards(HttpAppleAuthGuard) 
  @ApiResponse({status: HttpStatus.OK, type: HttpRestApiResponseLoggedInUser})
  public async appleLogin(@Req() request : HttpRequestWithAppleUser) : Promise<CoreApiResponse<HttpLoggedInUser>> {
    return CoreApiResponse.success(this.authService.login(request.user))
  }
}